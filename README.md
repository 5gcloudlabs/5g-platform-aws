...
| **Cloudflare Account & Domain** | A Cloudflare account with a **registered domain**, its **Zone ID**, and a **Cloudflare API Token** with **DNS Edit** permissions for that zone. <br><br> Follow Cloudflare’s official guide to create an API token: https://developers.cloudflare.com/fundamentals/api/get-started/create-token/ |
