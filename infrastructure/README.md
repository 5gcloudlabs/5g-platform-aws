...
All infrastructure is deployed using OpenTofu
 — an open-source Infrastructure as Code (IaC) tool fully compatible with Terraform syntax.
...
All AWS and Kubernetes resources are provisioned automatically, including cluster add-ons, namespaces, and GitOps integration via Argo CD.

📘 Notes

This infrastructure layer is designed to support 5G Core (Free5GC
) and UERANSIM
 workloads as part of the AWS 5G Cloud Labs
 project.

All Helm releases and Kubernetes manifests are managed declaratively through Argo CD once the cluster is provisioned.
